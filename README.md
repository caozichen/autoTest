...
- 新增“表单多语言 AI 翻译、发布与分享校验”脚本，覆盖简体中文、繁体中文和 English 的 AI 翻译、保存回读、发布、分享及公开页校验；自动化配置可将前序脚本输出的 `formId` 映射为本步骤的 `FORM_ID`。
- 完善流水线变量传递，支持通过 `sourceScriptId + sourcePath -> targetKey` 将同批次前序输出注入后续步骤；例如将发布脚本的 `formId` 映射到 `FORM_ID`。
- 增加“部分通过”结果：脚本正常完成但存在失败断言时以 `#FFD700` 展示，并允许流水线继续；超时、阻断或运行异常才记为“执行失败”。
- 优化运行记录交互与展示：顶部“全部通过 / 部分通过 / 执行失败”支持点击筛选和再次点击取消，筛选区支持一键重置；截图本地路径可直接在系统文件夹中定位并选中文件。
- 新增平台 Logo，并将“自动化测试平台”名称同步应用于侧栏、登录页、浏览器页签标题和 favicon。
- 增强运行稳定性与诊断：运行记录持续落盘并实时同步进度，强制停止支持等待浏览器清理，Runner 会隔离脚本未处理的异步错误，并补充接口与资源响应断言。
...
| `form-multilingual-translation-publish` | `scripts/form-multilingual-translation-publish.ui.spec.mjs` | 为简体中文原文启用繁体中文和 English，执行一键 AI 翻译，逐单元校验编辑器与接口，验证保存回读、完成发布、分享渠道及公开页三语言。 | 会真实调用 AI 翻译和完成发布；保存探针会在发布前自动恢复原译文。默认表单 ID 为 `4J027Q`，AI 等待总超时为 120 秒。 |
...
流水线只登录一次。步骤参数映射使用 `sourceScriptId + sourcePath -> targetKey`，提取出的字符串、数字或布尔值只在当前批次内生效，不会污染全局 Token。断言未全部通过但脚本已正常完成时记录为“部分通过”，保存输出并继续后续步骤；执行失败或必需路径无法提取时，流水线立即停止。
