...
- **创建表单设置断言（2026-09-16）**：普通与内地创建入口共用基础设置、消息与触达、分享与传播检查，覆盖保存请求、独立回读、发布后配置及通知模板；三个环境最新六步骤批次均全部通过，详见下文验证记录。
...
- **多语言适配（2026-09-14）**：适配新增的 `promotion_link`、`activity_location` 不可翻译分区，按表单配置严格核对清单，并区分当前测试与生产环境契约；英文目标支持 `English`、`英文`、`英语`、`英語` 标签。
- **功能一致性修复（2026-09-14）**：修复再次保存登录态丢失失效时间、最终保存期间停止未反映到历史终态、脚本错误搜索遗漏，以及 Runner 离线后主页仍保留在线状态的问题。
- **显示一致性（2026-09-14）**：各页面日期时间统一为浏览器本地时间 `YYYY-MM-DD HH:mm:ss`；实际被裁切的可见文本支持原生悬停提示，保留已有提示，跳过输入框和可编辑区域。
...
再次打开“管理登录态”会回填已保存的失效时间，保留秒和毫秒。仅修改备注或直接保存时保留原始时间戳，避免夏令时重复时段的转换歧义；主动清空可移除手动失效时间，JWT 自带过期时间仍然生效。
...
创建脚本现在共用 `scripts/support/form-settings-content.mjs` 中的协议、邮件、反馈和推广文案，追加用户协议（不要求填写前阅读）、提交成功分类标签、管理者及用户邮件通知、自定义富文本反馈、百度推广链接、新建分享渠道和发布前预览。每次运行把最终保存值与参数放入 `formContract.settings`，并保存独立 `form-settings.json` 附件；原 `FORM_ID`、`FORM_CONTRACT` 映射保持兼容。分类和标签各取首项，缺一种时仅配置另一种，两种都没有时记录失败断言并跳过该项；内地入口暂时注释停用这条可选项存在断言，双空只记录跳过，香港及测试环境规则不变。有数据时仍正常配置并验证保存结果。通知保留默认接收人、触发条件和邮件类型；预览只检查加载与接口资源，不填写表单。创建超时为标准 10 分钟，香港继续应用环境倍率。

新增设置采用实际保存请求与独立详情回读验证，产品接口拒绝时不会静默跳过或伪造成功。后续填写可能触发已开启的通知；当前部署关闭填写前阅读时协议渲染器没有额外复选框，已有填写脚本保持兼容，回归覆盖新增契约、反馈及推广。

协议语言兼容：测试环境字典可能返回 `enum_key=zh_cn`、`enum_value=zh_CN`，页面误用前者会被接口拒绝。保存协议时按本次真实字典，仅对已确认的简中、繁中、英文小写键转换为规范值；仅作用于本次表单的一次协议保存请求，其他配置保持原值。记录 `settings.agreement.compatibility` 中的原始与转发语言，网络记录保留实际发送正文，并严格检查真实响应和独立回读。兼容后流程通过不代表页面语言问题已修复。

发布会把通知正文转存到独立通知模板资源，因此发布前严格比较草稿配置，发布后额外读取 `notification/template`，核对 `customized` 中本次邮件的完整标题、正文和接收人；不能使用系统默认模板兜底。实际匹配模板记录在 `formContract.settings.publishedNotifications`，最终附件同步更新。修改共享脚本支持文件后，应等待当前任务全部结束，再重启本地 Runner，避免 Node 模块缓存导致继续运行旧版依赖。
...
`form-multilingual-translation-publish` 根据表单配置计算 `other_translation` 的预期清单，继续严格比较完整数组；缺项、重复项、未知项或顺序变化都会产生失败断言。`form_structure` 与 `email_translation` 保持既有断言。

2026-09-14 新版契约新增以下判断：

| 分区 | 进入不可翻译清单的条件 | 对应内容 |
| --- | --- | --- |
| `promotion_link` | 推广功能配置缺失或未启用 | 提交成功页底部推广按钮文案；链接地址不翻译。 |
| `activity_location` | 表单不是活动（`is_activity` 不为 `1`） | 活动主地点的详细地址。 |

同版新增的 `weixin_share` 没有启用开关，始终不应出现在不可翻译清单中。其余分区也按对应功能开关计算；提交按钮缺失启用标记时按启用处理。

| 运行环境／变量 | 使用的契约 |
| --- | --- |
| `TEST` 及其他环境代码 | 默认 `20260914` |
| `CN_PROD`、`HK_PROD` | 默认 `20260914`，对应 2026-09-16 批次确认的新版生产部署 |
| `TRANSLATION_SECTION_CONTRACT` | 可在脚本运行变量中显式设置为 `legacy` 或 `20260914`，优先于环境默认值；其他值在启动浏览器前报错。 |

环境默认值是当前部署约定，不会从待测响应自动识别或降级版本。2026-09-16 已将内地、香港默认基线同步为新版；仅在明确验证旧部署时显式设置 `TRANSLATION_SECTION_CONTRACT=legacy`。启动日志会记录本次使用的 `translationSectionContract`。
...
- 最终结果保存期间仍可停止；已受理的停止会以 `interrupted` 落盘，再释放批次占用。补写失败时维持执行中／等待保存状态，仅重试保存；终态已提交的完成批次拒绝再次停止（HTTP 409），已中断批次可幂等停止。
...
- 关键词搜索包含批次信息、脚本名称、入口文件和批次／脚本错误摘要，忽略大小写，并与环境和状态筛选组合；不搜索日志或断言全文。
- 详情请求慢或失败不会阻塞列表刷新；详情包含步骤日志、断言、接口证据和截图。

运行概览分别读取脚本、环境、运行记录和 Runner 健康状态。单项加载失败时显示相应警告，其他可用来源继续展示；健康检查失败显示离线，整体加载异常时清除旧快照。刷新恢复后会清除不可用提示，加载失败的数据不会显示成正常的零值或“暂无数据”。
...
2026-09-14 功能修复与多语言契约适配的校验结果：

| 校验阶段 | 结果 |
| --- | --- |
| 五项功能修复后完整测试 | Web 294 项、API／脚本／Chrome 353 项，共 647 项通过；生产构建通过。 |
| 随后完成多语言契约适配 | 多语言模块 49 项、注册契约 1 项，共 50 项通过；新增 6 组回归在修复前均可复现失败，修复后通过。 |
| 三个环境历史响应回放 | TEST、CN_PROD、HK_PROD 各 17 条源表单断言，全部通过。 |
| 新版契约 TEST 实际运行 | `RUN-20260914-134002`，单个多语言脚本通过；11,275 条断言全部通过，包含原失败的 `other_translation` 断言。 |

TEST 本次单脚本验证耗时约 2 分 16 秒，繁体中文、英文各完成 106/106 项翻译并发布，三语言公开预览和分享页切换通过，7 张证据截图已核对。

此前五项修复后的三个完整自动化批次也已执行：内地 `RUN-20260914-68C66F` 为 6/6 通过；测试 `RUN-20260914-7BF1EE` 为 4 步通过、2 步部分通过；香港 `RUN-20260914-3955C7` 为 5 步通过、1 步部分通过。此次契约适配消除了 TEST 多语言清单差异，随后仅重跑该多语言脚本；历史批次状态不变，TEST 提报列表和 HK 地区树响应收尾未完成的观察仍保留，不能据此将三个完整批次表述为全部通过。

上述测试分别对应修复阶段，未将不同轮次重复计数为一次全量结果。完整日志和验证报告保存在本地 `outputs/five-fixes-20260914/`、`outputs/translation-contract-20260914/`，运行记录与截图仍按原目录保存，不随 Git 提交。

### 2026-09-16 创建设置与三环境回归

两个创建入口通过 `scripts/support/form-post-collection-settings.mjs` 共用设置流程，覆盖范围如下：

| 设置范围 | 操作与断言 |
| --- | --- |
| 基础设置 | 保存用户协议名称和富文本正文，不要求填写前阅读；按提交成功行为配置首个可用分类、标签，核对保存参数和独立回读。 |
| 管理者通知 | 开启邮件通知，填写本次运行专属标题和正文；保留默认接收人、触发条件及通知类型，核对保存请求、界面回显和持久化结果。 |
| 消息与触达 | 配置用户自动化邮件、自定义富文本反馈和底部推广；校验正文、样式、链接、上传图片以及完整保存参数。 |
| 分享与传播 | 创建本次运行专属分享渠道，检查名称、长短链、服务器标识、启用状态及刷新回显。 |
| 发布前预览 | 等待表单挂载、接口正文、图片及字体加载完成，检查网络健康，保存截图和网络证据后关闭预览。 |
| 发布后核对 | 对照实际保存值检查协议、分类标签、反馈和推广；邮件从自定义通知模板独立回读标题、正文及默认参数。 |

`form-settings.json` 保存设置证据，并将设置输出到 `formContract.settings`；预览另外保存 `form-settings-preview.png` 和 `form-settings-preview-network.json`。邮件模板校验不代表实际投递成功，预览加载检查不代表逐项视觉检查，也不实际提交表单或点击推广链接。内地环境分类和标签均无选项时允许跳过，其他环境仍保留失败断言。

断言报告记录 `recorded-expect` 的 `expect`；`flowExpect` 用于阻断无法继续的流程，不会自动逐条计入报告，部分预览检查通过汇总断言记录。源码调用位置数、运行断言条数和业务检查点数不能等同；共用函数、循环及环境分支会影响实际数量。

香港繁体目标 `zh_HK` 的工作区和公开预览质量检查允许“户／戶”字形差异，兼容 OpenCC 香港字形输出；其他现有繁简检查、英文混入中文检查以及译文保存一致性检查仍保留。该例外只调整测试容许范围，不修改产品译文或历史批次结果。

调整后的完整真实批次结果：

| 环境 | 批次 | 脚本结果 | 断言数 | 失败断言 |
| --- | --- | --- | --- | --- |
| 测试 TEST | `RUN-20260916-E542B4` | 6/6 通过 | 15,520 | 0 |
| 内地 CN_PROD | `RUN-20260916-586B79` | 6/6 通过 | 15,479 | 0 |
| 香港 HK_PROD | `RUN-20260916-CA9B69` | 6/6 通过 | 15,460 | 0 |

多语言模块本地回归 84 项全部通过，另补测了公开预览的字形例外及其他简体残留仍被拒绝。运行记录、接口响应与截图保留在本地 `data/run-records/` 和 `outputs/artifacts/`，不随 Git 提交。历史部分通过批次不改写，以上结论以新批次为准。

提交前完整测试中 Web 294 项通过，API／Chrome 449 项中 446 项通过、3 项暴露测试夹具的旧成功页匹配问题。同步手动创建夹具与新增反馈页面模板后，该模块 8 项定向重跑全部通过；未再次重复整个全量测试。类型检查、生产构建和 `git diff --check` 均通过。
...
脚本配置中的超时时间保留为标准值。运行环境标识包含 `hk`（不区分大小写）时，脚本总超时以及各脚本的点击、弹窗、导航、断言、接口读取、AI 翻译和浏览器关闭等待上限均按标准值乘以 3。其余环境保持标准值。倍率按每次运行隔离计算，不改变轮询频率或重试次数，也不会在传递剩余等待时间时重复放大。

网络观察器的收尾等待单独设置：香港环境从 6 秒延长到 15 秒，测试和内地环境仍为 2 秒。请求一旦完整结束就立即继续，不会固定等待满额；即使已返回 HTTP 200，只要请求或正文仍未完成，超时后仍保留失败证据。
...
地区树 `/api/area/tree` 同样属于点击前置条件，需等待完整正文与界面更新后才能继续。“检查并编辑提报信息”还在进入编辑前、编辑态加载后、提交前及主动刷新前等待当前后台接口正文和页面依赖的图片、脚本、样式及字体完成，避免香港慢请求被保存或刷新提前打断。第三方遥测、后台 AI 状态轮询及事件流不作为这些操作的前置条件，但仍保留网络采集证据。每个等待节点最多使用一个导航预算（测试/内地 45 秒，香港 135 秒），同时受脚本总超时约束；未完成则停止后续操作，不重复已提交的更新。等待支持强制停止，并记录当前等待阶段与上限。
