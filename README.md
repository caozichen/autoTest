...
AutoTest 是一个本地优先的自动化测试管理平台。v1.1 提供 Vue 3 + Element Plus 管理端、本地 Playwright Runner、环境登录与 Token 注入、脚本管理、顺序流水线和真实运行记录；当前不依赖数据库。
...
- 刷新或关闭管理页面不会主动停止 Runner 中的当前任务；Runner 会独立保存结束结果。后续流水线步骤仍依赖前端调度，刷新后不会自动续跑整个流水线。重新打开“运行记录”后仍可从列表或详情强制停止批次。
...
截至 2026-09-11，本地历史中“测试表单冒烟测试”“内地表单兼容回归”“香港表单冒烟测试”均有六步骤全部通过记录。
这表示对应批次曾通过，后续运行仍以实际断言、网络和 AI 服务结果为准。
...
当 Runner 已无任务且前端也停止更新记录，系统保留 2 分钟宽限期用于步骤切换及变量校验：
全部步骤都有结果则补写成功、部分通过或失败终态；仍有未确认步骤则标记中断，保留已完成结果并提示人工核对业务数据。
重启后同样保留宽限期；尚未登记 Runner 任务、所有步骤仍排队的登录阶段继续沿用 4 小时兜底。

当前只完成失联恢复第一阶段：页面刷新不妨碍当前脚本的结果保存，但整条流水线仍由前端调度，不会自动续跑后续步骤。
