...
| `CN_PROD`、`HK_PROD` | 默认 `legacy`，对应 2026-09-14 已验证的生产部署 |
| `TRANSLATION_SECTION_CONTRACT` | 可在脚本运行变量中显式设置为 `legacy` 或 `20260914`，优先于环境默认值；其他值在启动浏览器前报错。 |

环境默认值是当前部署约定，不会从待测响应自动识别或降级版本。内地、香港生产环境升级到新版后，应设置 `TRANSLATION_SECTION_CONTRACT=20260914` 或同步更新默认契约，避免仍按旧清单验证。启动日志会记录本次使用的 `translationSectionContract`。
